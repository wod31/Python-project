            ...
            for chain2 in chains2:
                ...
                if similar_chains[chain1.id] == similar_chains[chain2.id]:
                    ...
                    if len(clashes) >= num_contact:
                        ...
                        io = PDBIO()
                        io.set_structure(str3[0])
                        io.save("selected_pdbs3/dimer%s.pdb" % id(str(3)))
                        return impose_clash(str3, strs, 2, i+1, 10, c, similar_chains)
...
for f in files_list:
    ...
    chains = list(structures[f][0].get_chains())
    chains[0].id = "R" + str(i)
    i += 1
    chains[1].id = "R" + str(i)
...
for index1, items1 in enumerate(structures_iter):
    ...
    if chains2[0] in similar_chains and chains2[1] in similar_chains:
        if similar_chains[chains2[0]] == similar_chains[chains2[0]] and similar_chains[chains2[1]] == similar_chains[
            chains2[1]]:
            structures.pop(file1)

strs = list(structures.values())

impose_clash(strs[0], strs, 2, 0, 10, 0, similar_chains)
